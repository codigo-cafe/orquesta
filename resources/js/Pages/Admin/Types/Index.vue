<template>
	<app-layout>
		<div class="col-12 mt-2">
			<div class="card">
				<div class="card-body">
					<h6 class="card-title text-dark text-gradient">Lista de Tipos de Personas</h6>
					<div class="d-flex justify-content-end" v-if="hasAnyPermission(['create_types'])">
						<button type="button" class="btn btn-dim" @click="openModal">
							<i class="fas fa-plus"></i> Registrar Tipo de Persona
						</button>
					</div>
					<div class="toolbar">
...
import ModalCreate from '@/Pages/Admin/Types/Create'
import ModalEdit from '@/Pages/Admin/Types/Edit'
...
export default {
	...
	methods: {
		...
		openModal() {
			if (this.hasAnyPermission(['create_types'])) {
		...
		edit(data) {
			var mensaje = '';
			if (this.hasAnyPermission(['edit_categories'])) {
				axios.get(this.route('types.edit', data.id))
				.then(response => {
		...
		restore(data) {
			if (this.hasAnyPermission(['restore_types'])) {
				this.data = data;
				this.options.url = 'types.restore';
				this.options.message = 'Está seguro de habilitar el tipo de persona: ';
		...
		destroy(data) {
			if (this.hasAnyPermission(['delete_types'])) {
				this.data = data;
				this.options.url = 'types.destroy';
				this.options.message = 'Está seguro de inhabilitar el tipo de persona: ';
				...
				self.table = $('#datatables').DataTable({
					...
					},
					{
						...
						"render": function(data,type,full){
							...
							if (data.status == 1 && data.can.delete_types) {
							...
							if (data.status == 0 && data.can.restore_types) {
