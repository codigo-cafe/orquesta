<template>
	<app-layout>
		<div class="col-12 mt-2">
			<div class="card">
				<div class="card-body">
					<h6 class="card-title text-dark text-gradient">Lista de Integrantes</h6>
					<div class="d-flex justify-content-end" v-if="hasAnyPermission(['create_members'])">
						<button type="button" class="btn btn-dim" @click="openModal">
							<i class="fas fa-plus"></i> Registrar Integrante
						</button>
					</div>
					<div class="toolbar">
		...
		<!-- Modal Create -->
		<modal-create :people="people" :instruments="instruments" :reload="reload" :showNotification="showNotification"></modal-create>
		<modal-edit :people="people" :instruments="instruments" :data="data" :reload="reload" :showNotification="showNotification"></modal-edit>
		<modal-confirm :data="data" :options="options" :reload="reload" :showNotification="showNotification"></modal-confirm>
	</app-layout>
</template>
<script>
import AppLayout from '@/Pages/Admin/Layouts/AppLayout'
import ModalCreate from '@/Pages/Admin/Members/Create'
import ModalEdit from '@/Pages/Admin/Members/Edit'
...
export default {
	...
	methods: {
		...
		openModal() {
			if (this.hasAnyPermission(['create_members'])) {
				axios.get(this.route('members.create'))
				.then(response => {
		...
		edit(data) {
			var mensaje = '';
			if (this.hasAnyPermission(['edit_members'])) {
				axios.get(this.route('members.edit', data.member.id))
				.then(response => {
		...
		restore(data) {
			if (this.hasAnyPermission(['restore_members'])) {
				this.data = data;
				this.options.url = 'members.restore';
				this.options.message = 'Está seguro de habilitar al integrante: ';
		...
		destroy(data) {
			if (this.hasAnyPermission(['delete_members'])) {
				this.data = data;
				this.options.url = 'members.destroy';
				this.options.message = 'Está seguro de inhabilitar al integrante: ';
				...
				self.table = $('#datatables').DataTable({
					...
					},
					{
						...
						"render": function(data,type,full){
							...
							if (data.status == 1 && data.can.delete_categories) {
							...
							if (data.status == 0 && data.can.restore_categories) {
