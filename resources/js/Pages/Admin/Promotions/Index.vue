<template>
	<app-layout>
		<div class="col-12 mt-2">
			<div class="card">
				<div class="card-body">
					<h6 class="card-title text-dark text-gradient">Lista de Promociones</h6>
					<div class="d-flex justify-content-end" v-if="hasAnyPermission(['create_promotions'])">
						<button type="button" class="btn btn-dim" @click="openModal">
							<i class="fas fa-plus"></i> Registrar Promoción
						</button>
					</div>
					<div class="toolbar">
...
import ModalCreate from '@/Pages/Admin/Promotions/Create'
import ModalEdit from '@/Pages/Admin/Promotions/Edit'
...
export default {
	...
	methods: {
		...
		openModal() {
			if (this.hasAnyPermission(['create_promotions'])) {
		...
		edit(data) {
			var mensaje = '';
			if (this.hasAnyPermission(['edit_promotions'])) {
				axios.get(this.route('promotions.edit', data.id))
				.then(response => {
		...
		restore(data) {
			if (this.hasAnyPermission(['restore_promotions'])) {
				this.data = data;
				this.options.url = 'promotions.restore';
				this.options.message = 'Está seguro de habilitar la promoción: ';
		...
		destroy(data) {
			if (this.hasAnyPermission(['delete_promotions'])) {
				this.data = data;
				this.options.url = 'promotions.destroy';
				this.options.message = 'Está seguro de inhabilitar la promoción: ';
				...
				self.table = $('#datatables').DataTable({
					...
					},
					{
						...
						"render": function(data,type,full){
							...
							if (data.status == 1 && data.can.delete_promotions) {
							...
							if (data.status == 0 && data.can.restore_promotions) {
