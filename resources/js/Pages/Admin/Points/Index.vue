...
import ModalCreate from '@/Pages/Admin/Points/Create'
import ModalEdit from '@/Pages/Admin/Points/Edit'
...
export default {
	...
	methods: {
		...
		openModal() {
			if (this.hasAnyPermission(['create_points'])) {
		...
		edit(data) {
			var mensaje = '';
			if (this.hasAnyPermission(['edit_points'])) {
				axios.get(this.route('points.edit', data.id))
				.then(response => {
		...
		restore(data) {
			if (this.hasAnyPermission(['restore_points'])) {
				this.data = data;
				this.options.url = 'points.restore';
				this.options.message = 'Está seguro de habilitar el punto de venta: ';
		...
		destroy(data) {
			if (this.hasAnyPermission(['delete_points'])) {
				this.data = data;
				this.options.url = 'points.destroy';
				this.options.message = 'Está seguro de inhabilitar el punto de venta: ';
				...
				self.table = $('#datatables').DataTable({
					...
					},
					{
						...
						"render": function(data,type,full){
							...
							if (data.status == 1 && data.can.delete_points) {
							...
							if (data.status == 0 && data.can.restore_points) {
