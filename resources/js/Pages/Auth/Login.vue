                ...
                <div class="card-body">
                    <form role="form text-left" @submit.prevent="submit" >
                        ...
                        <div class="form-group mb-3" :class="{ 'has-danger': errors.password }">
                            <input type="password" class="form-control" :class="{ 'is-invalid': errors.password }" v-model="form.password"  placeholder="Contraseña" aria-label="Password" aria-describedby="password-addon">
                            <div class="group-error" v-if="errors.password">
                                <small class="error text-danger">{{ errors.password }}</small>
                            </div>
                        </div>
                        <div class="form-check form-check-info text-left">
                            <input class="form-check-input" v-model="form.remember" type="checkbox" id="flexCheckDefault" checked>
                            <label class="form-check-label" for="flexCheckDefault">
                                Mantener mi sesión iniciada
                            </label>
                        </div>
                        <div class="text-center">
                            <button type="submit" class="btn btn-dim w-100 my-4 mb-2">Ingresar</button>
                        </div>
                        <p class="text-sm mt-3 mb-0">
                            <inertia-link v-if="canResetPassword" :href="route('password.request')" class="text-dark text-gradient font-weight-bolder">
