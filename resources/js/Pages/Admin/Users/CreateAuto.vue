<template>
	<div class="modal fade" id="ModalCreateAuto" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="ModalLabel" aria-hidden="true" ref="vuemodal">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						...
						<div class="alert alert-secondary" role="alert">
							<strong>Atención!</strong> Este modo de registro requiere que tenga una <strong>conexión activa a internet</strong> para poder enviar las credenciales de acceso al nuevo usuario registrado.
						</div>
					</div>
...
export default {
	...
	mounted() {
		var ModalCreateHidden = document.getElementById('ModalCreateAuto')
	...
	methods: {
		async submit() {
			this.disabled = true;
			axios.post(this.route('users.store-auto'), this.form)
			.then(response => {
				this.disabled = false;
				this.closeModal();
				this.reload();
		...
		closeModal() {
			var ModalCreateAuto = document.getElementById('ModalCreateAuto')
			var modal = bootstrap.Modal.getInstance(ModalCreateAuto)
			modal.hide();
		},
