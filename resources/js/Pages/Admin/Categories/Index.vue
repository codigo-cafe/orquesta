<template>
	<app-layout>
		<div class="col-12 mt-2">
			<div class="card">
				<div class="card-body">
					<h6 class="card-title text-dark text-gradient">Lista de Categorías</h6>
					<div class="d-flex justify-content-end" v-if="hasAnyPermission(['create_categories'])">
...
import ModalCreate from '@/Pages/Admin/Categories/Create'
import ModalEdit from '@/Pages/Admin/Categories/Edit'
...
export default {
	...
	methods: {
		...
		openModal() {
			if (this.hasAnyPermission(['create_categories'])) {
		...
		edit(data) {
			...
			if (this.hasAnyPermission(['edit_categories'])) {
				axios.get(this.route('categories.edit', data.id))
				.then(response => {
		...
		restore(data) {
			if (this.hasAnyPermission(['restore_categories'])) {
				this.data = data;
				this.options.url = 'categories.restore';
				this.options.message = 'Está seguro de habilitar la categoría: ';
		...
		destroy(data) {
			if (this.hasAnyPermission(['delete_categories'])) {
				this.data = data;
				this.options.url = 'categories.destroy';
				this.options.message = 'Está seguro de inhabilitar la categoría: ';
