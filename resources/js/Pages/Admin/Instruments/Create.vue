			...
			<form @submit.prevent="submit">
				<div class="modal-content">
					<div class="modal-header">
						<h6 class="modal-title" id="ModalLabel">Registrar Instrumento</h6>
					...
					<div class="modal-body">
						<div class="form-group bmd-form-group" :class="{ 'has-danger': errors.name }">
							...
							<input type="text" class="form-control" id="c_name" placeholder="Nombre del instrumento" v-model="form.name" autofocus>
						...
						<div class="row">
							<div class="col-md-12">
								<div class="form-group bmd-form-group" :class="{ 'has-danger': errors.classification_id }">
									<label for="c_classification_id">Clasificación</label>
...
export default {
	props: ['reload', 'showNotification', 'classifications'],
	...
	methods: {
		async submit() {
			this.disabled = true;
			axios.post(this.route('instruments.store'), this.form)
			.then(response => {
				this.disabled = false;
				this.closeModal();
				this.reload();
