<template>
	<app-layout>
		<div class="col-12 mt-2">
			<div class="card">
				<div class="card-body">
					<h6 class="card-title text-dark text-gradient">Lista de Clasificaciones</h6>
					<div class="d-flex justify-content-end" v-if="hasAnyPermission(['create_classifications'])">
						<button type="button" class="btn btn-dim" @click="openModal">
							<i class="fas fa-plus"></i> Registrar Clasificación
						</button>
					</div>
					<div class="toolbar">
...
import ModalCreate from '@/Pages/Admin/Classifications/Create'
import ModalEdit from '@/Pages/Admin/Classifications/Edit'
...
export default {
	...
	methods: {
		...
		openModal() {
			if (this.hasAnyPermission(['create_classifications'])) {
		...
		edit(data) {
			var mensaje = '';
			if (this.hasAnyPermission(['edit_classifications'])) {
				axios.get(this.route('classifications.edit', data.id))
				.then(response => {
		...
		restore(data) {
			if (this.hasAnyPermission(['restore_classifications'])) {
				this.data = data;
				this.options.url = 'classifications.restore';
				this.options.message = 'Está seguro de habilitar la clasificación: ';
		...
		destroy(data) {
			if (this.hasAnyPermission(['delete_classifications'])) {
				this.data = data;
				this.options.url = 'classifications.destroy';
				this.options.message = 'Está seguro de inhabilitar la clasificación: ';
			...
			this.$nextTick(() => {
				...
				self.table = $('#datatables').DataTable({
					...
					},
					{
						...
						"render": function(data,type,full){
							var botones = '';
							if (data.can.edit_classifications) {
							...
							if (data.status == 1 && data.can.delete_classifications) {
							...
							if (data.status == 0 && data.can.restore_classifications) {
