			...
			<form @submit.prevent="submit">
				<div class="modal-content">
					<div class="modal-header">
						<h6 class="modal-title" id="ModalLabel">Registrar Categoría</h6>
					...
					<div class="modal-body">
						<div class="form-group bmd-form-group" :class="{ 'has-danger': errors.name }">
							...
							<input type="text" class="form-control" id="c_name" placeholder="Nombre de la categoría" v-model="form.name" autofocus>
...
export default {
	...
	methods: {
		async submit() {
			this.disabled = true;
			axios.post(this.route('categories.store'), this.form)
			.then(response => {
				this.disabled = false;
				this.closeModal();
				this.reload();
