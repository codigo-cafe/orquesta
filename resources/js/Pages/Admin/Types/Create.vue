			...
			<form @submit.prevent="submit">
				<div class="modal-content">
					<div class="modal-header">
						<h6 class="modal-title" id="ModalLabel">Registrar Tipo de Persona</h6>
					...
					<div class="modal-body">
						<div class="form-group bmd-form-group" :class="{ 'has-danger': errors.name }">
							...
							<input type="text" class="form-control" id="c_name" placeholder="Nombre del tipo de persona" v-model="form.name" autofocus>
...
export default {
	...
	methods: {
		async submit() {
			this.disabled = true;
			axios.post(this.route('types.store'), this.form)
			.then(response => {
				this.disabled = false;
				this.closeModal();
				this.reload();
