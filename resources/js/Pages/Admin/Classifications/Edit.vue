			...
			<form @submit.prevent="submit">
				<div class="modal-content">
					<div class="modal-header">
						<h6 class="modal-title" id="ModalLabel">Modificar Clasificación</h6>
					...
					<div class="modal-body">
						<div class="form-group bmd-form-group" :class="{ 'has-danger': errors.name }">
							...
							<input type="text" class="form-control" id="e_name" placeholder="Nombre de la clasificación" v-model="form.name" autofocus>
...
export default {
	...
	methods: {
		async submit() {
			this.disabled = true;
			axios.put(this.route('classifications.update', this.classification.id), this.form)
			.then(response => {
				this.disabled = false;
				this.closeModal();
				this.reload();
