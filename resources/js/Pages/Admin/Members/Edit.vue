			...
			<form @submit.prevent="submit">
				<div class="modal-content">
					<div class="modal-header">
						<h6 class="modal-title" id="ModalLabel">Modificar Integrante</h6>
...
export default {
	...
	props: ['people', 'instruments', 'data', 'reload', 'showNotification'],
	...
	methods: {
		async submit() {
			this.disabled = true;
			axios.put(this.route('members.update', this.member.id), this.form)
			.then(response => {
				this.disabled = false;
				this.closeModal();
				this.reload();
		...
		fill() {
			...
			this.valuePerson = this.people.find( person => person.id === this.member.person_id );
			this.valueInstrument = this.instruments.find( instrument => instrument.id === this.member.instrument_id );
			this.onChangePerson(this.member.person);
			this.form.instrument_id = this.member.instrument_id;
			if (this.member.person.profile_photo_path) {
